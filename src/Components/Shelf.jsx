    ...
    <div className="max-w-screen h-fit p-4">
      ...
      {savedBooks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-y-10 gap-x-6">
          {savedBooks.map((book) => (
            <div key={book.key} className="card bg-white dark:bg-black shadow-md rounded-xl p-4">
              ...
              <button
                ...
                className="bg-red-500 text-white font-bold rounded p-2 hover:scale-105 transition-all cursor-pointer"
              >
                Remove
              </button>
            </div>
